...
class Azure:
    ...
    def getQueryResult(self) -> QUERY:
        ...
        """
        logger.info('Retrieving query result')
        ...
        logger.info('Retrieved query result')
        return response
    ...
    def getWorkItemDetails(self,item:int) -> WorkItem:
        ...
        logger.info(f"Retrieved {item} work item details")
        return WorkItem(
            payload.get('id'),
    ...
    def getWorkItems(self) -> list[WorkItem]:
        try:
            ...
            ids = [wi.id for wi in query.workItems]
            workItems = []
            logger.info(f"{len(ids)} work items identified. Retrieving details")
            for id in ids:
                details = self.getWorkItemDetails(id)
                workItems.append(details)
            logger.info('Work items details retrieved.')
            return workItems
        except Exception:
